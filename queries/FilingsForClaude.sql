select * from highConceptCoverageSubmissionsSample;
with
    topLevelParentsSum as (
        ...
        from
            ...
            inner join highConceptCoverageSubmissionsSample hccs
            on hccs.cik = fd.cik and hccs.accessionNumber = fd.accessionNumber and hccs.prefix = fd.prefix
        GROUP BY
            fd.prefix,
            fd.units,
            cth.cik,
...
FROM
    ...
    ) sc ON sc.cik = fd.cik and sc.accessionNumber = fd.accessionNumber and sc.prefix = subs.prefix 
ORDER BY
    cth.keyStatementRole,
    cth.ancestor,
    cth.arcWeight DESC;